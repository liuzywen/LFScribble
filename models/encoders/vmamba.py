...
if True:
    ...
    class SelectiveScan(torch.autograd.Function):
        ...
        def forward(ctx, u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=False, nrows=1):
            ...
            # print(type(C))
            # print(type(D))
            # print(type(delta_bias))
            # print(type(delta_softplus))
...
class FocalMambaBlock(nn.Module):
    ...
    def __init__(
        ...
        self.dropout = nn.Dropout(dropout) if dropout > 0. else nn.Identity()
        self.drop_path = DropPath(drop_path)
    def forward(self, x_focal: torch.Tensor):
        # 输入B,H,W,C
        x_focal_linear = self.in_proj(x_focal)
        x_focal_trans = x_focal_linear.permute(0, 3, 1, 2).contiguous()
        x_focal_conv = self.act(self.conv2d(x_focal_trans))  # (b, d, h, w)
        b, d, h, w = x_focal_trans.shape

        y_focal =self.op(x_focal_conv.view(b/12, d, h*w*12).permute(0, 2, 1))
        y_focal = y_focal.permute(0, 2, 1).view(b, h, w, d)

        x_focal_squeeze = self.avg_pool(x_focal_trans).view(b, d)
        x_focal_exitation = self.fc1(x_focal_squeeze).view(b, d, 1, 1).permute(0, 2, 3, 1).contiguous()  # b, 1, 1, d

        y_focal = y_focal * x_focal_exitation
        y_out = self.dropout(self.out_proj(y_focal))

        x = x_focal + self.drop_path(y_out)

        return x


class ChangeSSM(nn.Module):
    def __init__(
            self,
            # basic dims ===========
            d_model=96,
    ...
    def forward(self, x: torch.Tensor, change_x: torch.Tensor):
        selective_scan = selective_scan_fn_v1
        B, L, d = x.shape
        x = x.permute(0, 2, 1)
        change_x = change_x.permute(0, 2, 1)
        x_dbl = self.x_proj(rearrange(x, "b d l -> (b l) d"))  # (bl d)
        dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        dt = self.dt_proj.weight @ dt.t()
        dt = rearrange(dt, "d (b l) -> b d l", l=L)
...
class ChangeMambaBlock(nn.Module):
    ...
    def forward(self, x_all: torch.Tensor, x_focal: torch.Tensor):
        # 输入B,H,W,C
        x_focal_linear = self.in_proj(x_focal)
        x_focal_trans = x_focal_linear.permute(0, 3, 1, 2).contiguous()
        x_focal_conv = self.act(self.conv2d(x_focal_trans))  # (b, d, h, w)
        ...
        x_all_linear = self.in_proj(x_all)
        x_all_trans = x_all_linear.permute(0, 3, 1, 2).contiguous()
        x_all_conv = self.act(self.conv2d(x_all_trans))  # (b, d, h, w)
        B, D, H, W = x_all_trans.shape

        # y_focal = self.op1(x_focal_conv.view(b, d, h * w).permute(0, 2, 1))
        # y_focal = y_focal.permute(0, 2, 1).view(b, h, w, d).permute(0, 3, 1, 2)
        y_change_focal = self.op2(x_all_conv.view(b, d, h * w).permute(0, 2, 1), x_focal_conv.view(b, d, h * w).permute(0, 2, 1))
        y_change_focal = y_change_focal.permute(0, 2, 1).view(b, h, w, d)

        # y_change_focal = self.dropout(self.out_proj(y_change_focal))
        x_focal = x_focal + self.drop_path(y_change_focal)

        #新加的
        # x_rgb_squeeze = self.avg_pool(x_all_trans).view(b, d)
        # x_rgb_exitation = self.fc1(x_rgb_squeeze).view(b, d, 1, 1).permute(0, 2, 3, 1).contiguous()  # b, 1, 1, d
        # y_rgb = y_change_focal * x_rgb_exitation
        # x_focal = x_focal + self.drop_path(y_rgb)
...
# =====================================================
class Multi_Modal_SS2D_SSM(nn.Module):
    '''
...
class FusionMambaBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.drop_path = DropPath(drop_path)
        self.change_y_mamba = ChangeMambaBlock(
            d_model=128 * (2 ** 1),
        )
        self.mlp_branch = mlp_ratio > 0
        if self.mlp_branch:
    ...
    def _forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        x = (x_rgb + x_e + self.drop_path(self.op(x_rgb, x_e)) + self.drop_path(self.change_y_mamba(x_rgb, x_e))
             + self.drop_path(self.change_y_mamba(x_e, x_rgb)))
        # x = x_rgb + x_e + self.drop_path(self.change_y_mamba(x_rgb, x_e))
